...
async function run() {
  ...
  try {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const nome =
        faker && faker.person && faker.person.fullName
          ? faker.person.fullName()
          : faker && faker.name && faker.name.findName
            ? faker.name.findName()
            : "Usuario " + (i + 1);
      ...
      const usuario = await db.Usuario.create(
        { nome, email, senha_hash: "...", ativo: true },
        { transaction: t },
      );
      if (i < 30) {
        ...
        let dt = null;
        try {
          if (faker && faker.date && faker.date.past)
            dt =
              typeof faker.date.past === "function"
                ? faker.date.past(18)
                : null;
        } catch (e) {}
        await db.Aluno.create(
          { id_usuario: usuario.id_usuario, data_nascimento: dt },
          { transaction: t },
        );
      } else {
        let dt = null;
        try {
          if (faker && faker.date && faker.date.past)
            dt =
              typeof faker.date.past === "function" ? faker.date.past(5) : null;
        } catch (e) {}
        await db.Professor.create(
          { id_usuario: usuario.id_usuario, data_admissao: dt },
          { transaction: t },
        );
      }
    }
    ...
    for (const nome of disciplinasList) {
      const [d] = await db.Disciplina.findOrCreate({
        where: { nome },
        defaults: { nome, carga_horaria: 60 },
        transaction: t,
      });
    ...
    for (const curso of cursos) {
      for (const sufixo of ["A", "B"]) {
        ...
        turnoIdx++;
        const [tm] = await db.Turma.findOrCreate({
          where: { nome, ano_letivo: ano },
          defaults: { nome, ano_letivo: ano, turno, ativa: true },
          transaction: t,
        });
        turmas.push(tm);
      }
    ...
    if (professorIds.length === 0) {
      console.warn(
        "Nenhum professor encontrado — pulando vinculação de turma-disciplinas",
      );
    }
    for (const tm of turmas) {
      // pick 4..6 unique disciplines
      const count = Math.min(
        disciplinaObjs.length,
      ...
      for (const d of chosen) {
        const profId =
          professorIds.length > 0 ? randChoice(professorIds) : null;
        if (!profId) continue;
        await db.TurmaDisciplina.findOrCreate({
          where: { id_turma: tm.id_turma, id_disciplina: d.id_disciplina },
          defaults: {
    ...
    await t.commit();
    console.log(
      "Seed completa — disciplinas, turmas e vinculações adicionadas",
    );
    process.exit(0);
  } catch (e) {
    await t.rollback();
